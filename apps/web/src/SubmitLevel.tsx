...
const variantDefaults: Record<VariantKind,string> = {
  ORIGINAL:'Original', NERFED:'Nerfed', BUFFED:'Buffed', KEYLIMIT:'10K', NO_KEY_LIMIT:'No Key Limit', CUSTOM:'',
}
...
export function SubmitLevelPage() {
  ...
  const ja = locale === 'ja'
  const copy = ja ? {
    eyebrow: 'LEVEL SUBMISSION', title: '譜面を投稿', help: '投稿はまずスタッフ確認に入ります。承認後にLevel / Variant / Versionとして登録されますが、Rating Queueには自動投入されません。',
    login: '投稿するにはログインしてください。', loginButton: 'ログイン / アカウント作成', song: '曲名', artist: 'アーティスト名', creator: '制作者 / チーム名', effecter: 'エフェクター（任意）', variant: 'Variant', variantName: 'Variant名', keyLimit: 'キー上限', version: 'Version', download: '配布URL（任意）', video: '動画URL（任意）', sha: 'SHA-256（任意）', notes: '補足（任意）', submit: '投稿する', pendingLimit: '同時に保留できる投稿は5件までです。', history: '自分の投稿', none: 'まだ投稿はありません。', withdraw: '取り下げ', review: 'スタッフメモ', approved: '登録されたLevelを見る', sent: '投稿しました。スタッフ確認待ちです。', failed: '投稿に失敗しました', loading: '読み込み中…',
  } : {
    eyebrow: 'LEVEL SUBMISSION', title: 'Submit a level', help: 'Submissions are reviewed by staff first. Approval creates an ELF Level / Variant / Version, but does not automatically place it in the Rating Queue.',
    login: 'Log in to submit a level.', loginButton: 'Login / create account', song: 'Song', artist: 'Artist', creator: 'Creator / team', effecter: 'Effecter (optional)', variant: 'Variant', variantName: 'Variant name', keyLimit: 'Key limit', version: 'Version', download: 'Download URL (optional)', video: 'Video URL (optional)', sha: 'SHA-256 (optional)', notes: 'Notes (optional)', submit: 'Submit', pendingLimit: 'You may have up to 5 pending submissions at once.', history: 'My submissions', none: 'No submissions yet.', withdraw: 'Withdraw', review: 'Staff note', approved: 'View created Level', sent: 'Submitted. It is now waiting for staff review.', failed: 'Submission failed', loading: 'Loading…',
  }
  ...
  const [variantKind,setVariantKind]=useState<VariantKind>('ORIGINAL'); const [variantName,setVariantName]=useState('Original'); const [variantKeyLimit,setVariantKeyLimit]=useState('')
  const [versionLabel,setVersionLabel]=useState('v1.0'); const [downloadUrl,setDownloadUrl]=useState(''); const [videoUrl,setVideoUrl]=useState(''); const [sha256,setSha256]=useState(''); const [notes,setNotes]=useState('')
  ...
  const changeVariant=(kind:VariantKind)=>{setVariantKind(kind);setVariantName(variantDefaults[kind]);if(kind!=='KEYLIMIT')setVariantKeyLimit('')}
  const submit=async()=>{
    setBusy(true);setError('');setMessage('')
    try{
      await api('/submissions',{method:'POST',body:JSON.stringify({song,artist,creator,effecter:effecter||null,variantKind,variantName,variantKeyLimit:variantKind==='KEYLIMIT'?Number(variantKeyLimit):null,versionLabel,downloadUrl:downloadUrl||null,videoUrl:videoUrl||null,sha256:sha256||null,notes:notes||null})})
      setSong('');setArtist('');setCreator('');setEffecter('');setVariantKind('ORIGINAL');setVariantName('Original');setVariantKeyLimit('');setVersionLabel('v1.0');setDownloadUrl('');setVideoUrl('');setSha256('');setNotes('');setMessage(copy.sent);await loadMine()
  ...
  return <section className="submission-page">
    ...
    <div className="submission-grid">
      <div className="panel submission-form">
        ...
        <label>{copy.variant} *<select value={variantKind} onChange={(e)=>changeVariant(e.target.value as VariantKind)}><option value="ORIGINAL">Original</option><option value="NERFED">Nerfed</option><option value="BUFFED">Buffed</option><option value="KEYLIMIT">Keylimit</option><option value="NO_KEY_LIMIT">No Key Limit</option><option value="CUSTOM">Custom</option></select></label>
        <label>{copy.variantName} *<input maxLength={120} value={variantName} onChange={(e)=>setVariantName(e.target.value)}/></label>
        {variantKind==='KEYLIMIT'&&<label>{copy.keyLimit} *<input type="number" min="1" value={variantKeyLimit} onChange={(e)=>setVariantKeyLimit(e.target.value)}/></label>}
        ...
        <button disabled={busy||!song.trim()||!artist.trim()||!creator.trim()||!variantName.trim()||!versionLabel.trim()||(variantKind==='KEYLIMIT'&&(!variantKeyLimit||Number(variantKeyLimit)<1))} onClick={()=>void submit()}>{busy?copy.loading:copy.submit}</button>
      </div>
      <div className="panel"><h2>{copy.history}</h2>{rows.length?<div className="submission-list">{rows.map((row)=><article key={row.id}>
        <div className="title-row"><div><strong>{row.song}</strong><small>{row.artist} · {row.creator}</small><small>{row.variantName}{row.variantKeyLimit?` (${row.variantKeyLimit}K)`:''} · {row.versionLabel}</small></div><span className={`status ${row.status.toLowerCase()}`}>{row.status}</span></div>
