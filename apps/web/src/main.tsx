...
function ProposalForm({ levels, onCreated }: { levels: LevelListItem[]; onCreated:()=>void }) {
  const [levelId,setLevelId]=useState('');const [detail,setDetail]=useState<LevelDetail|null>(null);const [type,setType]=useState('RERATE');const [title,setTitle]=useState('');const [reason,setReason]=useState('');const [error,setError]=useState('')
  const [family,setFamily]=useState<Family>('G');const [tier,setTier]=useState(1);const [referenceId,setReferenceId]=useState('');const [technique,setTechnique]=useState('TECH');const [hint,setHint]=useState('');const [confidence,setConfidence]=useState('');const [notes,setNotes]=useState('')
  useEffect(()=>{if(!levelId){setDetail(null);setReferenceId('');return}void api<LevelDetail>(`/levels/${levelId}`).then((x)=>{setDetail(x);setReferenceId('');setFamily(x.currentRating?.family??'G');setTier(x.currentRating?.tier??1)})},[levelId])
  ...
  return <div className="panel"><h2>New proposal</h2><div className="form-grid"><label>Level<select value={levelId} onChange={(e)=>setLevelId(e.target.value)}><option value="">Select</option>{levels.map((l)=><option key={l.id} value={l.id}>{l.title}</option>)}</select></label><label>Type<select value={type} onChange={(e)=>{setType(e.target.value);setReferenceId('');setError('')}}>{['RERATE','REFERENCE_ADD','REFERENCE_MOVE','REFERENCE_REMOVE','METADATA','OTHER'].map((x)=><option key={x}>{x}</option>)}</select></label></div>
    ...
    {type==='REFERENCE_MOVE'&&selectedRef&&<><p className="note">Move preserves technique/confidence/notes and changes only slot/position. The current Reference row and canonical rating are captured as stale guards.</p><div className="form-grid"><label>Target family<select value={family} onChange={(e)=>setFamily(e.target.value as Family)}><option>P</option><option>G</option><option>U</option></select></label><label>Target tier<input type="number" min="1" max="30" value={tier} onChange={(e)=>setTier(Number(e.target.value))}/></label><label>Position<select value={hint} onChange={(e)=>setHint(e.target.value)}><option value="">No hint</option><option value="-2">lower</option><option value="-1">slightly lower</option><option value="0">center</option><option value="1">slightly higher</option><option value="2">higher</option></select></label></div></>}
    <input value={title} onChange={(e)=>setTitle(e.target.value)} placeholder="Proposal title"/><textarea value={reason} onChange={(e)=>setReason(e.target.value)} placeholder="理由・比較対象・根拠"/>{error&&<p className="error">{error}</p>}<button disabled={!canSubmit} onClick={()=>void create()}>Create</button></div>
}
