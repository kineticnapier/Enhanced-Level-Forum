...
if (!api.includes('decideProposal') || !api.includes('ProposalDecisionError')) throw new Error('admin proposal decision route is not using transactional proposal execution')
if (!api.includes('prepareReferenceProposalPayload') || !api.includes('ReferenceProposalError')) throw new Error('public Reference proposals must capture authoritative baselines at creation')

const services = await readFile(new URL('../apps/api/src/services.ts', import.meta.url), 'utf8')
if (!services.includes('publishCanonicalRatingInTransaction')) throw new Error('canonical rerate core must be reusable inside proposal decision transaction')

const proposalExecution = await readFile(new URL('../apps/api/src/proposals/execution.ts', import.meta.url), 'utf8')
for (const invariant of ['FOR UPDATE','currentCanonicalRating','targetLevelVersionId','proposedRating','Proposal baseline is stale','publishCanonicalRatingInTransaction','REFERENCE_PROPOSAL_TYPES','executeReferenceProposalInTransaction','PROPOSAL_EXECUTION']) {
  if (!proposalExecution.includes(invariant)) throw new Error(`proposal execution invariant missing: ${invariant}`)
}
if (!proposalExecution.includes("execution: 'STATUS_ONLY'")) throw new Error('non-executable proposal decisions must remain explicit status-only decisions')

const referenceProposals = await readFile(new URL('../apps/api/src/proposals/references.ts', import.meta.url), 'utf8')
for (const invariant of ['prepareReferenceProposalPayload','currentCanonicalRating','baselineReference','Reference proposal baseline is stale','PROPOSAL_ADD','PROPOSAL_MOVE','PROPOSAL_REMOVE','ON CONFLICT DO NOTHING']) {
...
if (!entry.includes("requireRole('MODERATOR')") || !entry.includes('createLevelFromTufObservation')) throw new Error('TUF create-level route must require MODERATOR and use reconciliation service')
...
if (!tufFetcher.includes('pass.levels.length !== pass.total')) throw new Error('TUF pagination must verify fetched count against source total')
if (!tufFetcher.includes('pass.pageTotals.every')) throw new Error('TUF pagination must reject a total that changes mid-scan')
...
if (tufImporter.includes('canonical_ratings') || tufImporter.includes('difficulty_references')) throw new Error('TUF importer must not write/read canonical ELF rating/reference tables')
if (!tufImporter.includes('external_id: issue.externalId')) throw new Error('TUF import issues must preserve external source IDs')
if (!tufImporter.includes("severity: 'INFO', kind: 'MISSING_REFERENCE_TYPE'")) throw new Error('missing TUF reference types should be informational source metadata')
...
if (tufReconciliation.includes('canonical_ratings') || tufReconciliation.includes('difficulty_references')) throw new Error('TUF reconciliation/create-level must not mutate canonical ELF rating/reference tables')
if (!tufReconciliation.includes('already mapped to a different ELF level')) throw new Error('TUF manual linking must reject silent external-ID remaps')
if (!tufReconciliation.includes('conflicts with the selected ELF version SHA-256')) throw new Error('TUF manual Version linking must reject known SHA conflicts')
if (!tufReconciliation.includes('Created') && !tufReconciliation.includes('INSERT INTO levels')) throw new Error('TUF reconciliation must be able to create an ELF Level')
...
for (const forbiddenMutation of ['INSERT INTO canonical_ratings','UPDATE canonical_ratings','DELETE FROM canonical_ratings','INSERT INTO difficulty_references','UPDATE difficulty_references','DELETE FROM difficulty_references']) {
...
if (!tufEvidence.includes('create proposals only from the latest TUF snapshot')) throw new Error('TUF evidence proposals must reject stale snapshot evidence')
if (!tufEvidence.includes('not a canonical P/G/U integer tier')) throw new Error('special/non-PGU TUF labels must not become canonical rerate proposals')
...
if ([web, webI18n, admin, adminI18n].some((source) => source.includes('AdoForum'))) throw new Error('legacy AdoForum branding remains in UI')
if (!(web + webI18n).includes('Enhanced Level Forum') || !(admin + adminI18n).includes('Enhanced Level Forum')) throw new Error('ELF branding missing')
if (!web.includes("type==='REFERENCE_ADD'") || !web.includes("type==='REFERENCE_MOVE'") || !web.includes("type==='REFERENCE_REMOVE'")) throw new Error('public proposal UI must build executable Reference proposal payloads')
if (!admin.includes('TufReconciliation') || !adminReconciliation.includes('/admin/imports/tuf/unlinked') || !adminReconciliation.includes('/admin/imports/tuf/link')) throw new Error('TUF reconciliation admin UI is not wired')
if (!adminReconciliation.includes('/admin/imports/tuf/create-level') || !adminReconciliation.includes("t('tuf.createButton')") || !admin.includes('canCreateLevel')) throw new Error('TUF create-level admin workflow is not wired with role gating')
if (!adminReconciliation.includes('TufEvidenceProposals') || !adminEvidence.includes('/admin/imports/tuf/evidence') || !adminEvidence.includes('/admin/imports/tuf/proposals')) throw new Error('TUF evidence proposal admin UI is not wired')
