...
if (thirtySix.footLaneCount !== 20) throw new Error('36K generic profile must mark the K17+ resources as foot inputs')

const naturalAlternation = estimateFingeringForKeyCount(
  { hitTimesMs: Array.from({ length: 20 }, (_, i) => i * 90) }, 4, { collectTrace: true, beamWidth: 128 },
)
...
const tripletRoll = estimateFingeringForKeyCount(
  { hitTimesMs: Array.from({ length: 24 }, (_, i) => i * 65) }, 3, { collectTrace: true, beamWidth: 160 },
)
...
const localBurstInput = { hitTimesMs: [
...
const speedChange = extractAdofaiPressEvents({ angleData: [0, 0, 0], settings: { bpm: 120, pitch: 100 }, actions: [
  { floor: 1, eventType: 'SetSpeed', speedType: 'Multiplier', bpmMultiplier: 2 }, { floor: 2, eventType: 'Twirl' },
] })
...
for (const filename of ['tile_unlit.png', 'planet-red.png', 'planet-blue.png', 'swirl_red.png', 'swirl_blue.png', 'SetSpeed.png', 'SpeedDown.png', 'tile_samespeed.png']) if (!assetReadme.includes(filename)) throw new Error(`replay asset documentation missing: ${filename}`)
const analyzerCli = await readFile(new URL('./analyze-fingering.mjs', import.meta.url), 'utf8')
for (const invariant of ['-result.json', '-replay.html', 'visualize-fingering.mjs', '--assets', '--output-dir', '--no-view']) if (!analyzerCli.includes(invariant)) throw new Error(`one-command analyzer workflow missing: ${invariant}`)
for (const moduleName of ['fingering-dp-profile.mjs', 'fingering-dp-state.mjs', 'fingering-dp-estimate.mjs', 'fingering-dp-analyze.mjs']) await readFile(new URL('./analyzer/' + moduleName, import.meta.url), 'utf8')
