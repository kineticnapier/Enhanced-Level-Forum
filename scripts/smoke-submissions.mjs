...
for (const invariant of ['SHA-256', '/submissions/mine', "method:'POST'", 'Rating Queue']) if (!publicPage.includes(invariant)) throw new Error(`public submission UI missing: ${invariant}`)
...
for (const invariant of ['/admin/submissions?status=', '/approve', '/reject', 'Rating Queueには自動投入されません']) if (!adminReview.includes(invariant)) throw new Error(`admin submission review missing: ${invariant}`)

console.log('LEVEL SUBMISSION STATIC SMOKE PASSED')
console.log('VIEWER submit -> staff review -> Level/Version; no automatic Rating Queue enqueue')
